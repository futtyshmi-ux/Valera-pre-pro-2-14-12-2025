...
export const generateProjectPPTX = async (project: ProjectData) => {
  ...
  slideTitle.addText(`PROJECT: ${project.meta.appName}`, {
  ...
  project.timeline.forEach((frame, index) => {
    ...
    // Title
    slide.addText(`SCENE ${index+1}: ${frame.title.toUpperCase()}`, {
    ...
    // Specs (Dur | Shot)
    const specs = `DUR: ${frame.duration || 4}s  |  SHOT: ${frame.shotType || 'N/A'}`;
    slide.addText(specs, {
    ...
    slide.addText(frame.description || "No description provided.", {
        x: COL_X, y: currY, w: COL_W, h: 1.2,
    ...
    const BOX_H = Math.max(REMAINING_H, 1.5); // Ensure min height
    ...
    let audioText = "";
    if (frame.dialogue) audioText += `"${frame.dialogue}"\n\n`;
    if (frame.speechPrompt) audioText += `(TTS: ${frame.speechPrompt})\n`;
    if (frame.musicMood) audioText += `Music: ${frame.musicMood}\n`;
    if (frame.sunoPrompt) audioText += `Suno: ${frame.sunoPrompt}`;
