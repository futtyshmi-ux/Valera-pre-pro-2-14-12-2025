...
const AssetCard = memo(({ 
    ...
    return (
        <div className={`bg-[#252525] rounded-xl border border-[#333] shadow-lg overflow-hidden flex flex-col transition-all duration-300 group hover:border-[#444] ${isExpanded && !isSidebarMode ? 'col-span-1 md:col-span-2 lg:col-span-2 2xl:col-span-2 ring-1 ring-[var(--accent)] z-10' : ''}`}>
            
            {/* 1. HEADER BAR */}
            <div className="h-10 border-b border-[#333] flex items-center justify-between px-3 bg-[#2a2a2a]">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                    <div className={`px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider flex items-center gap-1
                ...
                <div className="flex items-center gap-1">
                    {!isSidebarMode && (
                        <button onClick={() => onToggleExpand(char.id)} className="p-1.5 text-gray-500 hover:text-white hover:bg-[#333] rounded-lg transition-colors">
                    ...
                    <button 
                        ...
                        className={`p-1.5 rounded-lg transition-all cursor-pointer relative z-20 flex items-center justify-center gap-1
            ...
            <div className={`flex flex-col ${isExpanded && !isSidebarMode ? 'md:flex-row h-[500px]' : ''}`}>
                ...
                >
                    ...
                    {char.image ? (
                        <>
                            ...
                            <div className="absolute top-2 right-2 flex flex-col gap-1 opacity-0 group-hover/image:opacity-100 transition-all translate-x-2 group-hover/image:translate-x-0 z-20">
                                <button onClick={() => onEditImage(char)} className="p-2 bg-black/70 text-white rounded-lg hover:bg-[var(--accent)] backdrop-blur-md shadow-lg" title="Edit in Canvas"><Pencil size={14}/></button>
                                <button onClick={() => onDownload(char)} className="p-2 bg-black/70 text-white rounded-lg hover:bg-[var(--accent)] backdrop-blur-md shadow-lg" title="Download"><Download size={14}/></button>
                                <button onClick={() => onFillFrame(char)} className="p-2 bg-black/70 text-white rounded-lg hover:bg-[var(--accent)] backdrop-blur-md shadow-lg" title="AI Fill Frame"><Expand size={14}/></button>
                            </div>
                        ...
                        <div className="flex flex-col items-center justify-center text-[#444] gap-2 w-full h-full">
                            ...
                            <button onClick={() => onEditImage(char)} className="mt-2 px-3 py-1.5 bg-[#222] hover:bg-[var(--accent)] text-gray-400 hover:text-white rounded text-[10px] font-bold uppercase transition-colors border border-[#333]">
                                Open Canvas
                            </button>
                        </div>
                    )}
                    ...
                    <div className="absolute top-2 left-2 z-20 opacity-0 group-hover/image:opacity-100 transition-all -translate-x-2 group-hover/image:translate-x-0">
                       <label className="p-2 bg-black/70 hover:bg-[var(--accent)] text-white rounded-lg cursor-pointer backdrop-blur-md shadow-lg flex items-center justify-center">
                            <Upload size={14} />
                ...
                <div className={`flex flex-col p-3 gap-3 bg-[#1e1e1e] ${isExpanded && !isSidebarMode ? 'w-full md:w-1/3 overflow-y-auto custom-scrollbar' : 'flex-1'}`}>
                    ...
                    <div className={`flex-1 flex flex-col gap-1 min-h-[80px] relative group/prompt transition-all duration-300 ${isPromptExpanded ? 'z-50' : ''}`}>
                        ...
                        <div className="relative h-full">
                            <textarea 
                                ...
                                className={`w-full bg-[#151515] text-gray-300 text-xs p-2 rounded-lg border border-[#333] focus:border-[var(--accent)] focus:outline-none resize-y custom-scrollbar leading-relaxed
                                ${isPromptExpanded ? 'h-64 absolute top-0 left-0 z-50 shadow-2xl' : 'h-full'}`}
                            />
                            {/* Expand/Clear Controls */}
                            <div className="absolute top-2 right-2 flex flex-col gap-1 opacity-0 group-hover/prompt:opacity-100 transition-opacity bg-[#252525] rounded-lg border border-[#333] z-50 shadow-sm overflow-hidden">
                                <button 
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div className="flex flex-col gap-1">
                            ...
                            <input 
                                ...
                                placeholder="keyword"
                                className="bg-[#151515] text-gray-300 text-xs px-2 py-1.5 rounded-lg border border-[#333] focus:border-[var(--accent)] focus:outline-none"
                            />
                        </div>
                        <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2 pt-1">
                        <label className="p-2 bg-[#252525] hover:bg-[#333] border border-[#333] rounded-lg cursor-pointer text-gray-400 hover:text-white transition-colors" title="Add Reference Image">
                            <Paperclip size={14} />
                        ...
                        <button 
                            ...
                            className={`flex-1 py-2 rounded-lg font-bold uppercase text-[10px] tracking-wide flex items-center justify-center gap-2 shadow-lg transition-all
...
export const CharacterManager: React.FC<Props> = ({ characters, onUpdate, imageModel, isDriveConnected, onNotify, huggingFaceToken, isSidebarMode, currentProjectRatio }) => {
  ...
  const handleGenerate = useCallback(async (char: Character) => {
    ...
    try {
      ...
      if (char.description.length < 50) {
        let suffix = ", neutral background, highly detailed";
        if (char.type === 'character') suffix += ", character sheet, full body shot";
        if (char.type === 'item') suffix += ", product shot, isolated, 4k";
        if (char.type === 'location') suffix += ", wide angle, environmental concept art";
        promptToUse += suffix;
      }
  ...
  const QuickCreateButton = ({ type, icon, label, colorClass, desc }: any) => (
      <button 
        onClick={() => addNew(type)}
        className={`relative group flex flex-col items-center justify-center p-3 rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)] hover:bg-[var(--bg-card)] transition-all overflow-hidden ${isSidebarMode ? 'aspect-square' : 'flex-1 h-20'}`}
      >
    ...
    <div className={`animate-fade-in ${isSidebarMode ? '' : 'pb-20'}`}>
                  ...
                  {['all', 'character', 'location', 'item'].map(cat => {
                      ...
                      return (
                        <button
                            ...
                            className={`p-1.5 rounded-lg transition-all ${isActive ? 'bg-[var(--bg-card)] text-[var(--text-main)] shadow-sm' : 'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}
                            title={cat}
                        >
                            {icon}
                        </button>
      ...
      <div className={isSidebarMode ? "flex flex-col gap-4" : "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6"}>
        {filteredChars.length === 0 && (
            <div className="col-span-full py-8 flex flex-col items-center justify-center text-gray-600 opacity-50 border-2 border-dashed border-[#333] rounded-2xl bg-[#1a1a1a]">
