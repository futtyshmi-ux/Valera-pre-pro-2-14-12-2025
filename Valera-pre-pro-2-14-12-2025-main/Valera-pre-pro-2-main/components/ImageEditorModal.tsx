...
import { 
  ...
  Sliders, Check, Ratio, Aperture, Activity, Sun, Moon, Droplet, Thermometer, CircleDashed, Hash, Triangle, EyeIcon, Menu, LogOut, Filter
} from 'lucide-react';
...
const RESOLUTION_PRESETS = [
    { label: 'HD 16:9', w: 1920, h: 1080, icon: <Monitor size={12}/> },
    { label: 'Reels 9:16', w: 1080, h: 1920, icon: <Smartphone size={12}/> },
    { label: 'Square 1:1', w: 1080, h: 1080, icon: <Square size={12}/> },
    { label: '4K Ultra', w: 3840, h: 2160, icon: <LayoutTemplate size={12}/> },
    ...
    >
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
            ...
            <div className="flex-1 bg-[#151515] relative flex flex-col min-w-0 order-1 md:order-2">
                ...
                {(isCropMode || showResMenu) && (
                    <div className="bg-[#1a1a1a] border-b border-[#333] flex items-center justify-center p-2 gap-2 animate-fade-in-down z-10 flex-wrap">
                ...
                <div className="flex-1 relative overflow-hidden flex items-center justify-center bg-[#111]" ref={containerRef}>
                     <canvas 
                        ...
                        className={`w-full h-full block touch-none ${['brush', 'eraser', 'inpaint', 'lasso', 'pen'].includes(tool) ? 'cursor-none' : 'cursor-default'}`} 
                     />
                     
                     <div className="absolute bottom-20 md:bottom-4 right-4 flex gap-2 z-20">
                     ...
                     {isGenerating && (<div className="absolute inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-md"><div className="flex flex-col items-center gap-3"><RefreshCw size={48} className="animate-spin text-[var(--accent)]" /><span className="text-sm font-bold uppercase tracking-widest text-white">{genStatusMsg}</span></div></div>)}
                </div>
            </div>
        </div>

        {/* --- BOTTOM BAR (WITH ADJUSTMENTS STRIP) --- */}
        <div className="bg-[#202020] border-t border-[#333] flex flex-col z-30 shrink-0 relative order-3">
            
            {/* ADJUSTMENTS STRIP (Slides up) */}
            {showMobileAdjustments && selectedObject && (
                <div className="absolute bottom-full left-0 w-full bg-[#1e1e1e]/95 backdrop-blur border-t border-[#333] p-4 animate-fade-in-up z-20 shadow-2xl">
                    <div className="flex justify-between items-center mb-3">
                        <span className="text-[10px] font-bold uppercase text-[var(--accent)]">Color Adjustments</span>
                        <button onClick={() => setShowMobileAdjustments(false)} className="p-1"><X size={14}/></button>
                    </div>
                    <div className="grid grid-cols-2 gap-3 max-h-48 overflow-y-auto">
                        <div><label className="text-[9px] text-gray-400 flex items-center gap-1 mb-1"><Sun size={10}/> Exposure</label><input type="range" min="0" max="200" value={selectedObject.filters?.exposure ?? 100} onChange={(e) => updateObjectFilter('exposure', parseInt(e.target.value))} className="w-full h-1 bg-[#333] rounded appearance-none"/></div>
                        <div><label className="text-[9px] text-gray-400 flex items-center gap-1 mb-1"><Ratio size={10}/> Contrast</label><input type="range" min="0" max="200" value={selectedObject.filters?.contrast ?? 100} onChange={(e) => updateObjectFilter('contrast', parseInt(e.target.value))} className="w-full h-1 bg-[#333] rounded appearance-none"/></div>
                        <div><label className="text-[9px] text-gray-400 flex items-center gap-1 mb-1"><Droplet size={10}/> Saturate</label><input type="range" min="0" max="200" value={selectedObject.filters?.saturation ?? 100} onChange={(e) => updateObjectFilter('saturation', parseInt(e.target.value))} className="w-full h-1 bg-[#333] rounded appearance-none"/></div>
                        <div><label className="text-[9px] text-gray-400 flex items-center gap-1 mb-1"><Thermometer size={10}/> Warmth</label><input type="range" min="0" max="100" value={selectedObject.filters?.warmth ?? 0} onChange={(e) => updateObjectFilter('warmth', parseInt(e.target.value))} className="w-full h-1 bg-[#333] rounded appearance-none"/></div>
                        <div><label className="text-[9px] text-gray-400 flex items-center gap-1 mb-1"><CircleDashed size={10}/> Vignette</label><input type="range" min="0" max="100" value={selectedObject.filters?.vignette ?? 0} onChange={(e) => updateObjectFilter('vignette', parseInt(e.target.value))} className="w-full h-1 bg-[#333] rounded appearance-none"/></div>
                        <div><label className="text-[9px] text-gray-400 flex items-center gap-1 mb-1"><Hash size={10}/> Grain</label><input type="range" min="0" max="100" value={selectedObject.filters?.grain ?? 0} onChange={(e) => updateObjectFilter('grain', parseInt(e.target.value))} className="w-full h-1 bg-[#333] rounded appearance-none"/></div>
                    </div>
                </div>
            )}
            ...
            <div className="p-2 md:p-3 flex items-end gap-2">
                <div className="flex-1 bg-[#151515] border border-[#333] rounded-2xl flex items-center p-1.5 focus-within:border-[var(--accent)] transition-colors relative">
                    ...
                    {/* ADJUSTMENTS BUTTON (Mobile) */}
                    <button 
                        onClick={() => setShowMobileAdjustments(!showMobileAdjustments)} 
                        className={`p-2 rounded-xl transition-all mr-1 ${showMobileAdjustments ? 'bg-[var(--accent)] text-white' : 'text-gray-400 hover:text-white hover:bg-[#333]'}`}
                    ...
                    <button onClick={() => document.getElementById('char-ref-upload')?.click()} className="p-2 text-gray-400 hover:text-white rounded-xl hover:bg-[#333] transition-all"><Paperclip size={20}/><input id="char-ref-upload" type="file" className="hidden" accept="image/*" onChange={(e) => {
                        ...
                        if (file) {
                             ...
                             reader.onload = (ev) => {
                                 ...
                                 setReferences(prev => [...prev, { id: Date.now().toString(), src, orientation: 'landscape' }]);
                             };
                             reader.readAsDataURL(file);
                        }
                    }}/></button>
                    
                    {/* Prompt Textarea with Eraser */}
                    <div className="relative flex-1 group/prompt">
                        <textarea 
                            value={prompt} 
                            onChange={(e) => setPrompt(e.target.value)} 
                            placeholder="Describe edit (e.g. 'add neon lights')..." 
                            className="w-full bg-transparent text-sm text-gray-200 placeholder-gray-500 px-2 py-2 pr-8 focus:outline-none resize-none max-h-32 min-h-[40px] leading-relaxed custom-scrollbar"
                            style={{ height: '40px' }} 
                        />
                        {prompt && (
                            <button 
                                onClick={() => setPrompt('')} 
                                className="absolute right-1 top-1/2 -translate-y-1/2 p-1.5 text-gray-400 hover:text-white bg-black/20 hover:bg-red-500/20 rounded-full transition-all"
                                title="Clear Prompt"
                            >
                                <Eraser size={14}/>
                            </button>
            ...
            {references.length > 0 && (
                <div className="flex gap-2 px-3 pb-3 overflow-x-auto scrollbar-hide">
                    {references.map(ref => (
                        <div key={ref.id} className="relative group w-12 h-12 shrink-0 rounded-lg overflow-hidden border border-[#444]">
                            <img src={ref.src} className="w-full h-full object-cover" />
                            <button onClick={() => setReferences(p => p.filter(r => r.id !== ref.id))} className="absolute top-0 right-0 bg-black/60 text-white p-0.5"><X size={10}/></button>
                        </div>
                    ))}
                </div>
            )}
